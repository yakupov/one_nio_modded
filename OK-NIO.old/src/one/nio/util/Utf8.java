...
public final class Utf8 {
    ...
    public static int simulateWrite(String s) {
    	int length = s.length();
    	int size = 0;
        for (int i = 0; i < length; i++) {
            int v = s.charAt(i);
	...
	public static int write(String s, ByteBuffer buf) {
		int length = s.length();
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (v <= 0x7f && v != 0) {
            	...
            	buf.put((byte) (0xe0 | ((v >>> 12) & 0x0f)));
            	buf.put((byte) (0x80 | ((v >>> 6) & 0x3f)));
            	buf.put((byte) (0x80 | (v & 0x3f)));
            } else {
            	buf.put((byte) (0xc0 | ((v >>> 6) & 0x1f)));
            	buf.put((byte) (0x80 | (v & 0x3f)));
            }
        }
        return posBefore - buf.position();
	}
    ...
    public static String read(ByteBuffer buf, int start, int length) {
        char[] result = new char[length];
        int chars = 0;
        int end = start + length;
        for (int pos = start; pos < end; chars++) {
            ...
            if (b >= 0) {
                ...
                result[chars] = (char) ((b & 0x1f) << 6 | (buf.get(pos++) & 0x3f));
            } else {
                result[chars] = (char) ((b & 0x0f) << 12 | (buf.get(pos++) & 0x3f) << 6 | (buf.get(pos++) & 0x3f));
            }
        }
        return new String(result, 0, chars);
    }
